...
if($_SESSION['selfexpress_userid'] == 0){
	...
	if(isset($_GET['type']) && isset($_GET['id'])){
		...
		if(is_numeric($_GET['id'])){
			...
			if(in_array($_GET['type'], $allowed)){
				...
				$user_class = new User();
				$post->like_post($_GET['id'],$_GET['type'],$_SESSION['selfexpress_userid']);
 
				if($_GET['type'] == "user"){
					$user_class->follow_user($_GET['id'],$_GET['type'],$_SESSION['selfexpress_userid']);
		...
		$info = "";
				...
				if(isset($_SESSION['selfexpress_userid'])){
					...
					if(is_array($result)){
						...
						if(in_array($_SESSION['selfexpress_userid'], $user_ids)){
							$i_liked = true;
						}
					}
