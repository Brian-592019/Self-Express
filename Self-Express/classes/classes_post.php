...
class Post
{
			...
			if($this->error == ""){
				...
				if(isset($data['parent']) && is_numeric($data['parent'])){
					...
					if(is_array($mypost) && $mypost['userid'] != $userid){
						...
						//add notification
						add_notification($_SESSION['selfexpress_userid'],"comment",$mypost);
					}
	...
	public function i_own_post($postid,$selfexpress_userid)
	{
		...
		$result = $DB->read($query);
  		...
  		if(is_array($result)){
  			...
  			if($result[0]['userid'] == $selfexpress_userid){
	...
	public function like_post($id,$type,$selfexpress_userid){
			...
			if(is_array($result)){
				...
				if(!in_array($selfexpress_userid, $user_ids)){

					$arr["userid"] = $selfexpress_userid;
					...
					if($type != "user"){
						...
						//add notification
						add_notification($_SESSION['selfexpress_userid'],"like",$single_post);
					}

				}else{

					$key = array_search($selfexpress_userid, $user_ids);
					unset($likes[$key]);
